...
contract PulpBlue is ERC721, Ownable {
  ...
  function transfer(address _to, uint256 _tokenId) public payable virtual {
    require(msg.value >= 1000000000000000, "Minimum sale price is 0.001 ETH");
    require(_transferCounts[_tokenId] < 10, "Transfer count exceeded");

    if (_transferCounts[_tokenId] == 9) {
        uint256 contractBalance = address(this).balance;
        uint256 royalty = (contractBalance * royaltyPercentage) / 100;
        (bool success, ) = msg.sender.call{value: royalty}("");
        require(success, "Royalty transfer failed.");
        (success, ) = _to.call{value: contractBalance - royalty}("");
